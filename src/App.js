...
function App() {
  ...
  const totalSales     = allPurchases.reduce((s, p) => s + (p.total_amount || 0), 0);
  const totalPending   = allPurchases.filter(p => p.status !== 'paid').reduce((s, p) => s + (p.total_amount || 0), 0);
                ...
                <div className="stat-row">
                  <div className="stat-chip">
                    ...
                    <div>
                      ...
                      <div className="stat-chip-lbl">Customers</div>
                    </div>
                  </div>
                  <div className="stat-chip">
                    <span className="stat-chip-icon">🧾</span>
                    <div>
                      <div className="stat-chip-val">{totalOrders}</div>
                      <div className="stat-chip-lbl">Orders</div>
                    </div>
                  </div>
                  <div className="stat-chip stat-chip--green">
                    <span className="stat-chip-icon">💰</span>
                    <div>
                      <div className="stat-chip-val">₱{totalSales.toLocaleString('en-PH', {minimumFractionDigits:2})}</div>
                      <div className="stat-chip-lbl">Total Sales</div>
                    </div>
                  </div>
                  <div className="stat-chip stat-chip--amber">
                    <span className="stat-chip-icon">⏳</span>
                    <div>
                      <div className="stat-chip-val">₱{totalPending.toLocaleString('en-PH', {minimumFractionDigits:2})}</div>
                      <div className="stat-chip-lbl">Pending</div>
                    </div>
                  </div>
                </div>
              </section>
